...
public class WorkStealingMultiplier implements Multiplier {
	...
	public int[][] multiply(int[][] m1, int[][] m2) throws InterruptedException, ExecutionException {
		...
		List<Future<Work>> futureResult = pool.invokeAll(multiplierThreads);
		...
		for (Future<Work> future : futureResult) {
			column = i % m1Rows;
			result[row][column] = future.get().result;
			if (i + 1 == ((row + 1) * m1Rows))
				row++;
			i++;
		}
